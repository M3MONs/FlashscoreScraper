...
class TestFootballEventParser:
    def test_parse_event_returns_data(self, real_soup) -> None:
        result = FootballEventParser.parse_event(real_soup)
        assert result is not None
        assert isinstance(result, EventData)

    def test_date_is_not_default(self, real_soup) -> None:
        date = FootballEventParser.parse_event_date(real_soup)
        assert date != "Unknown date"
        assert len(date) > 5

    def test_two_teams_with_names(self, real_soup) -> None:
        participants = FootballEventParser.parse_event_teams(real_soup)
    ...
    def test_teams_have_images(self, real_soup) -> None:
        participants = FootballEventParser.parse_event_teams(real_soup)
        assert participants[0]["img"] is not None
        assert participants[1]["img"] is not None

    def test_teams_have_links(self, real_soup) -> None:
        participants = FootballEventParser.parse_event_teams(real_soup)
        assert participants[0]["link"] is not None
        assert participants[1]["link"] is not None

    def test_score_is_not_default(self, real_soup) -> None:
        score = FootballEventParser.parse_event_score(real_soup)
        assert score is not None
        assert score != "Unknown score"
        assert "-" in score

    def test_status_is_not_default(self, real_soup) -> None:
        status = FootballEventParser.parse_event_detail_status(real_soup)
        assert status is not None
        assert status != "Unknown status"
        assert len(status) > 0

    def test_league_exists(self, real_soup) -> None:
        league = FootballEventParser.parse_event_league(real_soup)
...
class TestFootballEventParserExactValues:
    ...
    def test_exact_date(self, real_soup) -> None:
        date = FootballEventParser.parse_event_date(real_soup)
        assert date == self.EXPECTED_DATE

    def test_exact_home_team(self, real_soup) -> None:
        participants = FootballEventParser.parse_event_teams(real_soup)
        home = participants[0]
    ...
    def test_exact_away_team(self, real_soup) -> None:
        participants = FootballEventParser.parse_event_teams(real_soup)
        away = participants[1]
    ...
    def test_exact_score(self, real_soup) -> None:
        score = FootballEventParser.parse_event_score(real_soup)
        assert score == self.EXPECTED_SCORE

    def test_exact_status(self, real_soup) -> None:
        status = FootballEventParser.parse_event_detail_status(real_soup)
        assert status == self.EXPECTED_STATUS

    def test_exact_league(self, real_soup) -> None:
        league = FootballEventParser.parse_event_league(real_soup)
        assert league is not None
